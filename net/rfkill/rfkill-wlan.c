...
int rockchip_wifi_set_carddetect(int val)
{
	...
	if (!strncmp(wifi_chip_type_string, "ap", 2) ||
		...
		irq_type = 1;
/*FIXME
 * mabye no need to call mmc_host_rescan()
 */
	return 0; //mmc_host_rescan(NULL, val, irq_type);//NULL => SDIO host
}
...
static int rockchip_wifi_voltage_select(void)
{
    ...
    if (1) { //rk3288 
		...
		regmap_write(mrfkill->pdata->grf, RK3288_GRF_IO_VSEL, 0x00100000);
	        LOG("%s: wifi & sdio reference voltage: 3.3V\n", __func__);
	    } else if (voltage  > 1500 && voltage < 1950) {
	        //writel_relaxed(0x00100010, RK_GRF_VIRT+0x380); //1.8
		regmap_write(mrfkill->pdata->grf, RK3288_GRF_IO_VSEL, 0x00100010);
